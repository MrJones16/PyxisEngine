...
struct PixelBodyElement {
    ...
    friend void to_json(json &j, const PixelBodyElement &pbe) {
        j = json{{"Element", pbe.element},
                 {"Hidden", pbe.hidden},
                 {"WorldPos", pbe.worldPos}};
    }
...
class PixelBody2D : public B2BodyNode {
  ...
  public:
    ...
    PixelBody2D(const std::string &name, b2BodyType type, World *world,
    ...
    ///
    /// This will re-create the b2body shapes & create new pixel bodies if
    /// split!
    /// </summary>
    ...
    /// <summary>
    /// takes the current map of elements of the pixel body, and creates the
    /// b2body fixtures and such based on them
    /// </summary>
  ...
  private:
    ...
    /// <summary>
    /// takes a set of points, and uses QueuePull to separate it into different
    /// contiguous sections
    /// </summary>
    /// <param name="source"></param>
    /// <returns></returns>
    std::vector<std::unordered_set<glm::ivec2, HashVector>>
    GetContiguousAreas(std::unordered_set<glm::ivec2, HashVector> source);

    /// <summary>
    /// Volatile extraction of a contiguous region from the bitmap, put into the
    /// result, starting from a position, using a seed fill approach
    /// </summary>
    /// <param name="startPos">a starting position to make the first region
    /// from, otherwise will grab first element in bitmap</param> <param
    /// name="result">the vector you supply to fill with a contiguous
    /// region</param> <param name="bitmap">the source that the region will be
    /// extracted from, volatile!</param>
    ...
    ///
    /// currently tries to grab as many diagonals as possible, but could be
    /// better to switch the two special cases to be inverted to ignore
    ...
    /// <summary>
    /// Gets the case for a marching squares algorithm, used by
    /// GetContourPoints;
    /// </summary>
    ...
    /// <summary>
    /// Uses a Douglass-Peucker line simplification to simplify points of the
    /// pixelbody perimeter to create a collision shape
    ///
    /// </summary>
    ...
    SimplifyPoints(const std::vector<p2t::Point> &contourVector, int startIndex,
