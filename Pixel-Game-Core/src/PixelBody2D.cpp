...
void PixelBody2D::Serialize(json &j) {
    ...
    PX_TRACE("Serializing position: ({0},{1})", GetPosition().x,
             GetPosition().y);
...
void PixelBody2D::Deserialize(json &j) {
    ...
    if (j.contains("m_DebugDisplay"))
        j.at("m_DebugDisplay").get_to(m_DebugDisplay);
...
void PixelBody2D::OnPhysicsUpdate() {
    /// when we update as a pixel body, we have to take all the elements of our
    /// body out of the simulation, and put them back in the newly calculated
    /// positions.
    ///
...
void PixelBody2D::OnRender() {
    if (m_DebugDisplay) {
        ...
        Renderer2D::DrawQuad({GetPosition().x, GetPosition().y, 20},
                             glm::vec2(0.75f, 0.75f), {1, 1, 1, 1});

        auto &T = m_B2Body->GetTransform();
        for (auto fixture = m_B2Body->GetFixtureList(); fixture != nullptr;
            ...
            for (int i = 0; i < shape->m_count - 1; i++) {
                ...
                glm::vec3 start =
                    glm::vec3(x1 * B2ToWorld, y1 * B2ToWorld, m_DebugDisplayZ);
                glm::vec3 end =
                    glm::vec3(x2 * B2ToWorld, y2 * B2ToWorld, m_DebugDisplayZ);
...
void PixelBody2D::EnterWorld() {
    ...
    for (auto &mappedElement : m_Elements) {
        ...
        if (ed.cell_type == ElementType::solid || e.m_Rigid) {
            ...
            if (std::abs(m_B2Body->GetAngularVelocity()) > 0.01f ||
                m_B2Body->GetLinearVelocity().LengthSquared() > 0.01f) {
...
void PixelBody2D::ExitWorld() {
    ...
    glm::ivec2 centerPixelWorld = {m_B2Body->GetPosition().x * B2ToWorld,
                                   m_B2Body->GetPosition().y * B2ToWorld};
    ...
    for (auto &mappedElement : m_Elements) {
        if (mappedElement.second
                .hidden) // make sure we aren't hidden before trying to pull
            continue;

        // the world position of the element is already known, so just try to
        // grab it
        Element &worldElement =
            m_PXWorld->GetElement(mappedElement.second.worldPos);
                ...
                .m_ID) // || !worldElement.m_Rigid TODO re-implement rigid?
        {
            ...
            if (elementData.cell_type == ElementType::solid ||
                ...
                // this could be the player replacing the blocks, or a solid
                // block reacts with something and stays solid, like getting
                // stained or something idk either way, in this situation we
                // just pull the new element
                ...
                // a solid, so we need to re-construct the rigid body without
                // that element! so we leave it in the sim, and erase the
                // previous from the body
                elementsToRemove.push_back(mappedElement.first);
            }
            // PX_TRACE("Element changed over update at ({0},{1})",
    ...
    // now that we pulled all the elements out, try to re-construct the body if
    // needed:
...
void PixelBody2D::GeneratePixelBody(bool SkipCalculations) {
    ...
    // getcontour points is able to have repeating points, which is a no-no for
    // box2d triangles / triangulation
    ...
    for (auto &triangle : triangles) {
        ...
        b2Vec2 points[3] = {{(float)((triangle->GetPoint(0)->x) * WorldToB2),
                             (float)((triangle->GetPoint(0)->y) * WorldToB2)},
                            {(float)((triangle->GetPoint(1)->x) * WorldToB2),
                             (float)((triangle->GetPoint(1)->y) * WorldToB2)},
                            {(float)((triangle->GetPoint(2)->x) * WorldToB2),
                             (float)((triangle->GetPoint(2)->y) * WorldToB2)}};
        if (points[0] == points[1] || points[0] == points[2] ||
            points[1] == points[2])
            continue; // degenerate polygon, so skip.
...
void PixelBody2D::Translate(const glm::vec2 &translation) {
    ...
    m_B2Body->SetTransform(
        b2Vec2(m_Position.x * WorldToB2, m_Position.y * WorldToB2), m_Rotation);
}
...
float PixelBody2D::GetRotation() { return m_B2Body->GetAngle(); }
...
glm::vec2 PixelBody2D::GetLocalPixelVelocity(glm::ivec2 localPosition) {
    ...
    float angle =
        std::atan2f(localPosition.y, localPosition.x) + m_B2Body->GetAngle();
    ...
    return (GetLinearVelocity() + tangentialDirection) * PPU *
           Physics2D::m_Step;
}

std::vector<std::unordered_set<glm::ivec2, HashVector>>
PixelBody2D::GetContiguousAreas(
    std::unordered_set<glm::ivec2, HashVector> source) {
...
void PixelBody2D::QueuePull(
    glm::ivec2 startPos, std::unordered_set<glm::ivec2, HashVector> &result,
    std::unordered_set<glm::ivec2, HashVector> &source) {
    if (source.size() == 0)
        return;
...
std::vector<p2t::Point> PixelBody2D::GetContourPoints() {
    ...
    glm::ivec2 cursor =
        glm::ivec2(halfWidth, halfHeight); // get top right local pos
        ...
        while (!m_Elements.contains(cursor)) {
            ...
            if (cursor.x < -halfWidth) {
                ...
                if (cursor.y < -halfHeight) {
                    // ran out of pixels to test for! so the entire array is
                    // air...
...
int PixelBody2D::GetMarchingSquareCase(glm::ivec2 localPosition) {
    /*
            ...
            checking the 2x2 pixel grid, assigning these values to each pixel,
       if not transparent
...
PixelBody2D::SimplifyPoints(const std::vector<p2t::Point> &contourVector,
                            int startIndex, int endIndex, float threshold) {
    float maxDist = 0.0f;
    int maxIndex = 0;
