...
class Texture {
  public:
    ...
    enum WrapMode { Repeat, RepeatMirrored, ClampToEdge, ClampToBorder };

    enum TextureMode { Stretch, Tile };
  ...
  public:
    ...
    virtual TextureSpecification &GetTextureSpecification() {
...
class Texture2D : public Texture {
  public:
    ...
    Create(uint32_t width, uint32_t height,
           TextureSpecification spec = TextureSpecification());
    static Ref<Texture2D>
    CreateGlyph(uint32_t width, uint32_t height, unsigned char *buffer,
                TextureSpecification spec = TextureSpecification());
...
class Texture3D : public Texture {
  public:
    static Ref<Texture3D>
    Create(const std::string &path,
           TextureSpecification spec = TextureSpecification());
    static Ref<Texture3D>
    Create(uint32_t width, uint32_t height, uint32_t length,
           TextureSpecification spec = TextureSpecification());
