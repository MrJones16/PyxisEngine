...
struct Message {
    ...
    Message(uint32_t headerID) { header.id = headerID; }

    Message(const std::string &compressedStr) {
        std::string uncompressed;
        snappy::Uncompress(compressedStr.data(), compressedStr.size(),
                           &uncompressed);
    ...
    void Compressed(std::string &compressedStrOut) {
        *this << header.id;
        ...
        // we need to pull the ID out so it doesn't mess with the message
        // unintentionally.
        uint32_t revertID;
        *this >> revertID;
        // PX_TRACE("Compressed Message. Compression: {0}%, [{1}]->[{2}]",
        // ((float)compressedStrOut.size() / (float)inString.size()) * 100.0f,
        // (float)inString.size(), (float)compressedStrOut.size());
    }
    ...
    /// <summary>
    /// Pushes any arbitrary data into the message buffer using a pointer and
    /// size
    ///
    /// Not Volatile
    /// </summary>
    ...
    friend Message &operator<<(Message &msg, const DataType &data) {
        ...
        static_assert(
            ...
            "Data is too complex to be pushed into message body vector");
    ...
    /// </summary>
    template <typename DataType>
    friend Message &operator<<(Message &msg,
    ...
    friend Message &operator>>(Message &msg, DataType &data) {
        ...
        static_assert(std::is_standard_layout<DataType>::value,
                      "Data is too complex to have been in a body vector");
    ...
    friend Message &operator>>(Message &msg, std::vector<DataType> &data) {
        ...
        // make sure we empty the array first, as we are supposed to be
        // replacing it... or maybe not?
        data.clear();
