#include <pxpch.h>
#include "SteamNetworkServer.h"

// Based on the GameNetworkingSockets example_chat.cpp
...
namespace Pyxis
{
	void GameServer::Start(uint16 nPort)
	{
		...
		if (m_hPollGroup == k_HSteamNetPollGroup_Invalid)
			PX_CORE_ERROR("Failed to listen on port {0}", nPort);
		PX_INFO("Server listening on port {0}", nPort);
	...
	void GameServer::OnSteamNetConnectionStatusChanged(SteamNetConnectionStatusChangedCallback_t* pInfo)
	{
		...
		case k_ESteamNetworkingConnectionState_ProblemDetectedLocally:
		{
			...
			if (pInfo->m_eOldState == k_ESteamNetworkingConnectionState_Connected)
			{
				...
				PX_CORE_WARN("Connection {0} {1}, reason {3}: {4}",
