...
void PixelCameraNode::Deserialize(json &j) {
    ...
    if (j.contains("m_Aspect"))
        j.at("m_Aspect").get_to(m_Aspect);
    if (j.contains("m_FOV"))
        j.at("m_FOV").get_to(m_FOV);
...
glm::vec2 PixelCameraNode::MouseToWorldPos(glm::vec2 mousePos) {
...
void PixelCameraNode::RecalculateProjectionMatrix() {
    float height = m_FOV * m_Aspect;
    m_ProjectionMatrix = glm::ortho(-m_Size.x / 2, m_Size.x / 2, -m_Size.y / 2,
                                    m_Size.y / 2, m_Near, m_Far);
...
const glm::mat3 PixelCameraNode::GetRotationMatrix() const {
    return glm::rotate(glm::mat4(1.0f), glm::radians(m_Rotation.x),
...
// void PixelCameraNode::SetLocalTransform(const glm::mat4& transform)
//{
// }

// void PixelCameraNode::Translate(glm::vec3 translation)
//{
//	Node(translation);
//	RecalculateViewMatrix();
// }

// void PixelCameraNode::Rotate(glm::vec3 rotation)
//{
//	m_Rotation += rotation;
