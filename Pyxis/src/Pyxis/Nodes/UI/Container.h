...
namespace UI {
/// <summary>
/// A UI Node that will update the positions of it's children to be in an
/// organized fashion
/// </summary>
  ...
  public:
    ...
    virtual void OnInspectorRender() override {
        ...
        if (ImGui::TreeNodeEx("Container", ImGuiTreeNodeFlags_DefaultOpen)) {
            // Size
            if (ImGui::DragFloat2("Size", glm::value_ptr(m_Size)))
                RearrangeChildren();

            if (ImGui::DragFloat("Gap", &m_Gap))
                RearrangeChildren();

            if (ImGui::Checkbox("Show Region", &m_ShowRegion))
                RearrangeChildren();

            if (ImGui::Button("Rearrange Children"))
                RearrangeChildren();

            if (ImGui::TreeNodeEx("Direction",
            ...
            if (ImGui::TreeNodeEx("Horizontal Alignment",
                                  ImGuiTreeNodeFlags_DefaultOpen)) {
                if (ImGui::Button("Left")) {
                    m_HorizontalAlignment = Direction::Left;
                    RearrangeChildren();
                }
                if (ImGui::Button("Right")) {
                    m_HorizontalAlignment = Direction::Right;
                    RearrangeChildren();
                }
                if (ImGui::Button("Center")) {
                    m_HorizontalAlignment = Direction::Center;
                    RearrangeChildren();
                }
                ImGui::TreePop();
            }
            if (ImGui::TreeNodeEx("Vertical Alignment",
                                  ImGuiTreeNodeFlags_DefaultOpen)) {
                if (ImGui::Button("Up")) {
                    m_VerticalAlignment = Direction::Up;
                    RearrangeChildren();
                }
                if (ImGui::Button("Down")) {
                    m_VerticalAlignment = Direction::Down;
                    RearrangeChildren();
                }
                if (ImGui::Button("Center")) {
                    m_VerticalAlignment = Direction::Center;
                    RearrangeChildren();
                }
                ImGui::TreePop();
            }
    ...
    virtual void OnRender() override {
        if (m_Enabled && m_ShowRegion) {
            if (m_TextureResource != nullptr) {
                ...
                glm::mat4 sizeMat =
                    glm::scale(glm::mat4(1.0f), {m_Size.x, m_Size.y, 1});

                // TODO: Test ordering
                Renderer2D::DrawQuadEntity(GetWorldTransform() * sizeMat,
    ...
    void RearrangeChildren() {
        ...
        float maxHorizontal = m_Size.x - (m_Padding * 2),
              maxVertical = m_Size.y - (m_Padding * 2);
        float sumVertical = -m_Gap, sumHorizontal = -m_Gap;

        // we need to set which corner to start in based on horizontal and
        // vertical alignment
        if (m_HorizontalAlignment == Left) {
            // shift left half of the size to reach left wall, and give padding
            // space
            startPosition += glm::vec3(-maxShiftX, 0, 0);
        } else if (m_HorizontalAlignment == Right) {
            startPosition += glm::vec3(maxShiftX, 0, 0);
        } else {
            // since we are centered, we have to find the total length of the
            // items, with a max of the border of the container
            if (m_Direction == Left || m_Direction == Right) {
                for (auto &child : m_Children) {

                    if (UIRect *rect = dynamic_cast<UIRect *>(child)) {
                        // keep adding a shift until a child is too big to fit
                        if (sumHorizontal + rect->m_Size.x + m_Gap >
                            maxHorizontal)
                            break;
            ...
            if (UIRect *rect = dynamic_cast<UIRect *>(child)) {
                ...
                case Pyxis::UI::Direction::Up:
                    if (position.y + (rect->m_Size.y) > maxShiftY) {
                        // with this next object, we will be past the vertical
                        // limit, so lets shift against alignment
                        if (m_HorizontalAlignment == Left)
                ...
                case Pyxis::UI::Direction::Up:
                case Pyxis::UI::Direction::Down: {
                    if (rect->m_Size.x + m_Gap > nextLineShift)
                        nextLineShift = rect->m_Size.x + m_Gap;
                }
                case Pyxis::UI::Direction::Left:
                case Pyxis::UI::Direction::Right: {
                    if (rect->m_Size.y + m_Gap > nextLineShift)
                        nextLineShift = rect->m_Size.y + m_Gap;
                }
                default:
                    break;
                }

                // since we have adjusted the start position, lets reset the
                // transform, and set it to the position for now, and then we
                // will move it based on its size
                rect->ResetLocalTransform();
                rect->Translate(position);

                // for both vertical & horizontal alignment, decide if we need
                // to shift a direction to offset the size of the object
