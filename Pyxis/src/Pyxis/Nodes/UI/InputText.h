...
class InputText : public InputBase {
  protected:
    // to track if we have created our own value (in this case string) so we
    // know to free the memory upon deletion
  ...
  public:
    InputText(const std::string &name, Ref<Font> font,
              std::string *value = nullptr)
        : InputBase(name), m_Font(font) {
    ...
    virtual void Deserialize(json &j) override {
        ...
        if (j.contains("m_TextColor"))
            j.at("m_TextColor").get_to(m_TextColor);
        if (j.contains("m_TextBorderSize"))
            j.at("m_TextBorderSize").get_to(m_TextBorderSize);
        if (j.contains("m_Font"))
            m_Font =
                ResourceManager::Load<Font>(j.at("m_Font").get<std::string>());
        if (j.contains("m_FontSize"))
            j.at("m_FontSize").get_to(m_FontSize);
        if (j.contains("m_RenderRect"))
            j.at("m_RenderRect").get_to(m_RenderRect);
        if (j.contains("m_Multiline"))
            j.at("m_Multiline").get_to(m_Multiline);
        if (j.contains("m_ScaleToFit"))
            j.at("m_ScaleToFit").get_to(m_ScaleToFit);
        if (j.contains("m_Alignment"))
            j.at("m_Alignment").get_to(m_Alignment);
    }
    ...
    virtual void OnInspectorRender() override {
        ...
        if (ImGui::TreeNodeEx("Text Input", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            ImGui::InputFloat2("Text Border Size",
                               glm::value_ptr(m_TextBorderSize));
    ...
    virtual void OnRender() override {
        ...
        if (m_Enabled) {
            ...
            float characterHeight =
                m_FontSize * m_Font->m_CharacterHeight * 1.3f;
            ...
            if (m_Multiline) {
                Renderer2D::DrawText(
                    ...
                    GetWorldTransform() *
                        glm::translate(glm::mat4(1),
                                       {-m_Size.x / 2,
                                        (m_Size.y / 2) - characterHeight,
                                        -0.0001f}),
                    m_Font, m_FontSize, 1.3f, m_Size.x - m_TextBorderSize.x,
                    m_Alignment, m_TextColor, m_UUID);
            } else {
                ...
                const char *indicator =
                    (s_MousePressedNodeID == m_UUID) && (ms / 500) % 2 == 0
                        ? "|"
                        : "";

                Renderer2D::DrawTextLine(
                    (*m_Value) + indicator,
                    GetWorldTransform() *
                        glm::translate(glm::mat4(1), {0, 0, -0.0001f}),
                    m_Font, maxSize, m_FontSize, m_Alignment, m_ScaleToFit,
                    m_TextColor, m_UUID);
            }
        }
    }
