...
class ChainBody2D : public B2BodyNode {
  public:
    ChainBody2D(const std::string &name, b2BodyType type)
        : B2BodyNode(name, type) {}

    ChainBody2D(UUID id) : B2BodyNode(id) {}
    ...
    void TransferWorld(b2World *world) override {
        if (m_HasBody) {
            // store data from body, then make a new one, then put the new data
            // in.
            ...
            while (fixture != nullptr) {
                b2ChainShape *chain =
                    dynamic_cast<b2ChainShape *>(fixture->GetShape());
                if (chain) {
                    json shapeData;
                    shapeData["Type"] = "ChainShape";
                    shapeData["VertexCount"] = chain->m_count;
                    for (int i = 0; i < chain->m_count; i++) {
                        shapeData["Vertices"].push_back(glm::vec2(
            ...
            for (auto &shape : shapes) {
                if (shape["Type"] == "ChainShape") {
                    ...
                    chain.CreateLoop((b2Vec2 *)vertices.data(),
                                     vertices.size() - 1);
                    b2FixtureDef fixtureDef;
    ...
    void DebugDraw(float scaling, float z) {
        ...
        for (auto fixture = m_B2Body->GetFixtureList(); fixture != nullptr;
            ...
            for (int i = 0; i < shape->m_count - 1; i++) {
                ...
                PX_CORE_TRACE("Drawing line from ({0},{1}) to ({2},{3})",
