...
class OrthographicCameraController {
  public:
    float m_ZoomLevel = 1;
    float m_CameraSpeed = 100;
    float m_Sensitivity = 0.5f;

    OrthographicCameraController(float width, float aspect, float nearClip,
                                 float farClip)
        : m_Camera(
              CreateRef<OrthographicCamera>(width, aspect, nearClip, farClip)) {
    ...
    void OnUpdate(Timestep ts) {
        ...
        else m_Camera->SetRotation(m_Camera->GetRotation() + glm::vec3(0,0,
        angle * ts.GetSeconds()));*/

        // if (direction.x != 0 || direction.y != 0 || direction.z != 0)
        // direction = glm::normalize(direction);
        m_Camera->SetPosition(m_Camera->GetPosition() +
                              direction * m_CameraSpeed * ts.GetSeconds());
    }
    ...
    void SetAspect(float aspectRatio) { m_Camera->SetAspect(aspectRatio); }
    ...
    void SetRotation(glm::vec3 rotation) { m_Camera->SetRotation(rotation); }
    ...
    void OnMouseScrolledEvent(MouseScrolledEvent &event) {
        if (event.GetYOffset() < 0) {
            if (Pyxis::Input::IsKeyPressed(PX_KEY_LEFT_SHIFT)) {
                ...
                m_ZoomLevel *= 1.1f;
                m_Camera->SetWidth(m_Size.x * m_ZoomLevel);
                m_Camera->SetHeight((m_Size.x * m_ZoomLevel) *
                                    m_Camera->GetAspect());
            }
        } else {
            if (Pyxis::Input::IsKeyPressed(PX_KEY_LEFT_SHIFT)) {
                // PX_CORE_INFO("FasterCameraSpeed");
                m_CameraSpeed *= 1.1f;
                if (m_CameraSpeed > 2.0f)
                    m_CameraSpeed = 2.0f;
            } else {
                // PX_CORE_INFO("SlowerCameraSpeed");
                m_CameraSpeed *= 0.9f;
                if (m_CameraSpeed < 0.1f)
                    m_CameraSpeed = 0.1f;
