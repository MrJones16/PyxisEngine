...
Application::Application(const std::string &name, uint32_t width,
                         uint32_t height, const std::string &iconPath) {

    PX_CORE_ASSERT(!s_Instance, "Application already exists!");
    s_Instance = this;

    Pyxis::WindowProps props = WindowProps(name, width, height, iconPath);
...
void Application::OnEvent(Event &e) {
    ...
    dispatcher.Dispatch<WindowCloseEvent>(
        PX_BIND_EVENT_FN(Application::OnWindowClose));
    dispatcher.Dispatch<WindowResizeEvent>(
        PX_BIND_EVENT_FN(Application::OnWindowResize));
    ...
    case Pyxis::EventType::WindowResize:
        EventSignal::s_WindowResizeEventSignal(
            static_cast<WindowResizeEvent &>(e));
        break;
    case Pyxis::EventType::WindowFocus:
        EventSignal::s_WindowFocusEventSignal(
            static_cast<WindowFocusEvent &>(e));
        break;
    ...
    case Pyxis::EventType::KeyReleased:
        EventSignal::s_KeyReleasedEventSignal(
    ...
    case Pyxis::EventType::MouseButtonPressed:
        EventSignal::s_MouseButtonPressedEventSignal(
            static_cast<MouseButtonPressedEvent &>(e));
        break;
    case Pyxis::EventType::MouseButtonReleased:
        EventSignal::s_MouseButtonReleasedEventSignal(
            static_cast<MouseButtonReleasedEvent &>(e));
        break;
    ...
    case Pyxis::EventType::MouseScrolled:
        EventSignal::s_MouseScrolledEventSignal(
            static_cast<MouseScrolledEvent &>(e));
        break;
    default:
        break;
    }
...
// Use when you need to remove the layer immediately, for use if your layer is
// not a pointer that can be deleted.
void Application::PopLayer(Ref<Layer> layer) { m_LayerStack.PopLayer(layer); }
