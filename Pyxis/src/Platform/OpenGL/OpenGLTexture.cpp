...
OpenGLTexture2D::OpenGLTexture2D(uint32_t width, uint32_t rows,
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, m_InternalFormat, m_Width, m_Height, 0,
                 m_DataFormat, GL_UNSIGNED_BYTE, buffer);
    // set texture options
    SetParametersFromSpecification();
}
...
void OpenGLTexture2D::SetData(void *data, uint32_t size) {
    ...
    PX_CORE_ASSERT(size == m_Width * m_Height * m_BytesPerPixel,
                   "Data must be entire texture");
    glTextureSubImage2D(m_RendererID, 0, 0, 0, m_Width, m_Height, m_DataFormat,
                        GL_UNSIGNED_BYTE, data);
}
void OpenGLTexture2D::SetPixel(int x, int y, uint32_t color) {
    PX_CORE_ASSERT(sizeof(color) == m_BytesPerPixel,
                   "SetPixel only works with RGBA aka 4 bytes. Add an overload "
                   "for your type!");
    glTextureSubImage2D(m_RendererID, 0, x, y, 1, 1, m_DataFormat,
