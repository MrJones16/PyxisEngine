...
OpenGLShader::PreProcess(const std::string &source) {
    ...
    while (typeStart != std::string::npos)        // while finding #type
    {
        ...
        PX_CORE_ASSERT(eol != std::string::npos,
                       "Syntax Error"); // assert not eof
        size_t begin =
            typeStart + typeTokenLength + 1; // set begin to "#type "<-
        std::string type =
            source.substr(begin, eol - begin); // find substring like "vertex"
        PX_CORE_ASSERT(ShaderTypeFromString(type),
        ...
        // if there is no #type, use end of file, else use next #type as end of
        // string
        shaderSources[ShaderTypeFromString(type)] = source.substr(
            nextLinePos,
            typeStart - (nextLinePos == std::string::npos ? source.size() - 1
                                                          : nextLinePos));
    }
...
void OpenGLShader::Compile(
    ...
    for (auto &kv : shaderSources) {
        ...
        if (success == GL_FALSE) {
            ...
            PX_CORE_ERROR("{0}|{1}", source, infoLog.data());
