...
static void AttachColorTexture(uint32_t id, int samples, GLenum internalformat,
    ...
    // glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index, id,
    // 0); might work? but not for multisample maybe?
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index,
                           TextureTarget(multisampled), id, 0);
}

static void AttachDepthRenderBuffer(uint32_t id, GLenum target, GLenum format,
...
OpenGLFrameBuffer::OpenGLFrameBuffer(const FrameBufferSpecification &spec)
    ...
    for (auto format : spec.Attachments.Attachments) {
        ...
        if (format.TextureType == FrameBufferTextureType::Color) {
            ...
            PX_CORE_ERROR("Frame Buffer was given an attachment with no type! "
...
void OpenGLFrameBuffer::Invalidate() {
        ...
        for (size_t i = 0; i < m_ColorAttachments.size(); i++) {
            ...
            case FrameBufferTextureFormat::RGBA8: {
                Utils::AttachColorTexture(
                    m_ColorAttachments[i], m_Specification.samples, GL_RGBA8,
                    GL_RGBA, GL_UNSIGNED_BYTE, m_Specification.Width,
            ...
            case FrameBufferTextureFormat::R32I: {
                Utils::AttachColorTexture(
                    ...
                    GL_RED_INTEGER, GL_INT, m_Specification.Width,
                    m_Specification.Height, i);
                break;
            }
            case FrameBufferTextureFormat::R32UI: {
                Utils::AttachColorTexture(
                    m_ColorAttachments[i], m_Specification.samples, GL_R32UI,
                    GL_RED_INTEGER, GL_UNSIGNED_INT, m_Specification.Width,
                    m_Specification.Height, i);
                break;
            }
            default:
    ...
    if (m_DepthAttachmentSpecification.TextureFormat !=
        ...
        case FrameBufferTextureFormat::DEPTH24STENCIL8: {
            Utils::AttachDepthRenderBuffer(
                m_DepthAttachment, GL_RENDERBUFFER, GL_DEPTH24_STENCIL8,
    ...
    if (m_ColorAttachments.size() > 1) {
        PX_CORE_ASSERT(m_ColorAttachments.size() <= 4,
...
void OpenGLFrameBuffer::ReadPixel(uint32_t attachmentIndex, int x, int y,
                                  void *data) {
    ReadPixels(attachmentIndex, x, y, 1, 1, data);
}

void OpenGLFrameBuffer::ReadPixels(uint32_t attachmentIndex, int x, int y,
                                   int width, int height, void *data) {
    PX_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size(),
                   "Attachment index too great!");
    ...
    case FrameBufferTextureFormat::R32UI: {
        glReadPixels(x, y, width, height, GL_RED_INTEGER, GL_UNSIGNED_INT,
                     data);
        break;
    }
    default: {
...
void OpenGLFrameBuffer::ClearColorAttachment(int index, const void *value) {
    PX_CORE_ASSERT(index < m_ColorAttachments.size(),
                   "Attachment index too great!");

    switch (m_ColorAttachmentSpecifications[index].TextureFormat) {
    case FrameBufferTextureFormat::RGBA8: {
        glClearTexImage(m_ColorAttachments[index], 0, GL_RGBA, GL_UNSIGNED_BYTE,
                        value);
        break;
    }
    case FrameBufferTextureFormat::R32I: {
        glClearTexImage(m_ColorAttachments[index], 0, GL_RED_INTEGER, GL_INT,
                        value);
        break;
    }
    case FrameBufferTextureFormat::R32UI: {
        glClearTexImage(m_ColorAttachments[index], 0, GL_RED_INTEGER,
