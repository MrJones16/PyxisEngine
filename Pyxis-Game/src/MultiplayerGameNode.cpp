...
MultiplayerGameNode::MultiplayerGameNode() : GameNode("Multiplayer Game Node") {
    ...
    m_LSButton = Instantiate<UI::TextButton>(
        "Okay Button", ResourceManager::Load<Font>("assets/fonts/Aseprite.ttf"),
        std::bind(&MultiplayerGameNode::ReturnToMenu, this));
    m_LSButton->m_PPU = 0.5f;
    m_LSButton->m_Text = "Okay";
    m_LSButton->m_TextColor =
        glm::vec4(255.0f / 255.0f, 221.0f / 255.0f, 159.0f / 255.0f, 1);
    ;
    m_LSButton->Translate({0, -64, -0.01});
    m_LSButton->m_TextureResource = ResourceManager::Load<Texture2DResource>(
        "assets/textures/UI/ButtonWide.png");
    m_LSButton->m_TextureResourcePressed =
...
void MultiplayerGameNode::Connect(SteamNetworkingIdentity &identity,
                                  int virtualPort) {
    // max out input tick so we can discard early sent mtc's
    m_InputTick = -1;
    m_MultiplayerState = MultiplayerState::Connecting;
        ...
        while (!m_MTCBuffer.empty()) {
            if (m_MTCBuffer.front().m_Tick == m_InputTick) {
                ...
                if (m_LastRequestedTick != m_InputTick) {
                    Network::Message requestMsg(static_cast<uint32_t>(
                        GameMessage::Client_RequestMergedTick));
...
void MultiplayerGameNode::OnFixedUpdate() {
    if (m_MultiplayerState == MultiplayerState::Connected) {
        ...
        mousePosMsg.header.id =
            static_cast<uint32_t>(GameMessage::Client_ClientDataMousePosition);
...
void MultiplayerGameNode::OnRender() {
    if (m_MultiplayerState == MultiplayerState::DownloadingWorld) {
        m_LSText->m_Text =
            "Downloading World: { " +
            std::to_string(((float)m_DownloadCount / (float)m_DownloadTotal) *
                           100.0f) +
            "% }";
    }
}
...
void MultiplayerGameNode::OnConnectionSuccess() {
    ...
    clientDataMsg.header.id =
        static_cast<uint32_t>(GameMessage::Client_ClientData);
    ...
    Network::Message msg;
    msg.header.id =
        static_cast<uint32_t>(GameMessage::Client_RequestAllClientData);
    ...
    while (PollMessage(msg)) {
        ...
        case GameMessage::Server_ClientDataMousePosition: {
            ...
            *msg >> otherClient;
            ...
            if (otherClient == k_HSteamNetConnection_Invalid) {
                ...
                *msg >> m_ClientDataMap[k_HSteamNetConnection_Invalid]
        ...
        case GameMessage::Server_AllClientData: {
            ...
            *msg >> numClients;
            ...
            // now that we recieved the other player data, lets request the game
            // data
            Network::Message requestGameDataMsg;
            requestGameDataMsg.header.id =
                static_cast<uint32_t>(GameMessage::Client_RequestGameData);
        ...
        case GameMessage::Server_GameDataInit: {
            ...
            *msg >> numPixelBodies >> numChunks;
            ...
            m_DownloadTotal = static_cast<uint64_t>(numPixelBodies) +
                              static_cast<uint64_t>(numChunks);
            m_DownloadCount = 0;

            PX_TRACE("Downloading game at tick [{0}], simulation tick [{1}]",
                     m_InputTick, m_World.m_SimulationTick);
            if (m_DownloadTotal == 0) {
                ...
                finishedDownloadMsg.header.id =
                    static_cast<uint32_t>(GameMessage::Client_GameDataComplete);
                ...
                PX_TRACE("Expecting {0} PixelBodies and {1} Chunks totalling "
                         "{2} messages",
                         numPixelBodies, numChunks, m_DownloadTotal);
            }
            break;
        }
        ...
        case GameMessage::Server_GameDataChunk: {
            ...
            // we recieved a pixel body, so lets count it, and add it to the
            // world
            m_DownloadCount++;
            ...
            if (m_DownloadCount == m_DownloadTotal) {
                ...
                gameDataCompleteMsg.header.id =
                    static_cast<uint32_t>(GameMessage::Client_GameDataComplete);
                SendMessageToServer(gameDataCompleteMsg);

                // now that we have finished recieving the game data and loading
                // the world, start the game!
                m_MultiplayerState = MultiplayerState::Connected;
                m_LSCanvas->m_Enabled = false;
                m_LSText->m_Enabled = false;
                m_LSButton->m_Enabled = false;
                ...
                gameDataRecievedMsg.header.id =
                    static_cast<uint32_t>(GameMessage::Client_GameDataRecieved);
                SendMessageToServer(gameDataRecievedMsg);
                // PX_INFO("Downloading World: [{0}%]", (double)m_DownloadCount
