...
GameNode::GameNode(std::string debugName)
    : Node(debugName), m_KeyPressedReciever(this, &GameNode::OnKeyPressedEvent),
    ...
    EventSignal::s_MouseButtonPressedEventSignal.AddReciever(
        m_MouseButtonPressedReciever);
    ...
    m_Hotbar->CreateTextures("assets/textures/UI/GreenCanvas/",
                             "GreenCanvasTile_", ".png");
    m_Hotbar->m_PPU = 32;
    ...
    m_PlayButton = Instantiate<UI::Button>(
        "Play Button", ResourceManager::Load<Texture2DResource>(
                           "assets/textures/UI/PlayButton.png"));
    m_PlayButton->SetFunction(std::bind(&GameNode::PlayButtonFunc, this));
    m_PlayButton->m_TextureResourcePressed =
        ResourceManager::Load<Texture2DResource>(
            "assets/textures/UI/PlayButtonPressed.png");
    m_PlayButton->m_Size = {32, 32};
    m_PlayButton->m_Enabled = false;
    ButtonHolder->AddChild(m_PlayButton);
    m_PauseButton = Instantiate<UI::Button>(
        "Pause Button", ResourceManager::Load<Texture2DResource>(
                            "assets/textures/UI/PauseButton.png"));
    m_PauseButton->SetFunction(std::bind(&GameNode::PauseButtonFunc, this));
    m_PauseButton->m_TextureResourcePressed =
        ResourceManager::Load<Texture2DResource>(
            "assets/textures/UI/PauseButtonPressed.png");
    ...
    auto brushCircle = Instantiate<UI::Button>(
        "BrushCircle", ResourceManager::Load<Texture2DResource>(
                           "assets/textures/UI/BrushCircleButton.png"));
    brushCircle->SetFunction(
        std::bind(&GameNode::SetBrushType, this, BrushType::circle));
    brushCircle->m_TextureResourcePressed =
        ResourceManager::Load<Texture2DResource>(
            "assets/textures/UI/BrushCircleButtonPressed.png");
    brushCircle->m_Size = {32, 32};
    brushCircle->Translate({-20, 0, 0});
    BrushOptions->AddChild(brushCircle);

    auto brushSquare = Instantiate<UI::Button>(
        "BrushSquare", ResourceManager::Load<Texture2DResource>(
                           "assets/textures/UI/BrushSquareButton.png"));
    brushSquare->SetFunction(
        std::bind(&GameNode::SetBrushType, this, BrushType::square));
    brushSquare->m_TextureResourcePressed =
        ResourceManager::Load<Texture2DResource>(
            "assets/textures/UI/BrushSquareButtonPressed.png");
    brushSquare->m_Size = {32, 32};
    brushSquare->Translate({20, 0, 0});
    BrushOptions->AddChild(brushSquare);
    container->AddChild(BrushOptions);

    auto ElementButtonContainer =
        Instantiate<UI::Container>("Element Buttons Container");
    ...
    for (int i = 0; i < ElementData::s_ElementData.size(); i++) {
        ElementProperties &ed = ElementData::GetElementProperties(i);
        ...
        auto ElementTextButton = Instantiate<UI::TextButton>(
            "ElementTextButton",
            ResourceManager::Load<Font>("assets/fonts/Aseprite.ttf"),
            std::bind(&GameNode::SetBrushElement, this, i));
        ElementTextButton->m_TextureResource =
            ResourceManager::Load<Texture2DResource>(
                "assets/textures/UI/TextPlateWhite.png");
        ElementTextButton->m_TextureResourcePressed =
            ResourceManager::Load<Texture2DResource>(
                "assets/textures/UI/TextPlateWhitePressed.png");
        ElementTextButton->m_TextBorderSize = glm::vec2(2);
        ElementTextButton->m_Color =
            glm::vec4(r, g, b, std::max(128, a)) / 255.0f;
        ElementTextButton->m_TextColor = glm::vec4(
            ElementTextButton->m_Color.r, ElementTextButton->m_Color.g,
            ElementTextButton->m_Color.b, 1);

        if (ElementTextButton->m_Color.r + ElementTextButton->m_Color.g +
                ElementTextButton->m_Color.b / 3.0f >
            0.8f) {
            // element color is bright so make it darker
            ElementTextButton->m_TextColor =
                glm::vec4(ElementTextButton->m_Color.r * 0.1f,
                          ElementTextButton->m_Color.g * 0.1f,
                          ElementTextButton->m_Color.b * 0.1f, 1);
        } else {
            // its dark so make it brighter
            // ElementTextButton->m_TextColor = ElementTextButton->m_Color;
            for (int i = 0; i < 3; i++)
                ElementTextButton->m_TextColor[i] =
                    std::min(ElementTextButton->m_TextColor[i] + 0.50f, 1.0f);
        }
    ...
    // Quit Button
    auto quitButton = Instantiate<UI::TextButton>(
        "Quit Game Button",
        ResourceManager::Load<Font>("assets/fonts/Aseprite.ttf"),
        std::bind(&GameNode::ReturnToMenu, this));
    quitButton->m_PPU = 0.5;
    quitButton->m_Text = "Quit Game";
    quitButton->m_TextColor =
        glm::vec4(255.0f / 255.0f, 221.0f / 255.0f, 159.0f / 255.0f, 1);
    quitButton->Translate({0, 0, 1});
    quitButton->m_TextureResource = ResourceManager::Load<Texture2DResource>(
        "assets/textures/UI/ButtonWide.png");
    quitButton->m_TextureResourcePressed =
        ResourceManager::Load<Texture2DResource>(
            "assets/textures/UI/ButtonWidePressed.png");
    ...
    screenSpace
        ->PropagateUpdate(); // simple way to tell the hotbar to fix itself
}

GameNode::~GameNode() {}

/// <summary>
/// This is the main update loop involving actually playing the game, input,
/// controls, ect. Also includes rendering loop
///
...
void GameNode::GameUpdate(Timestep ts) {
    ...
    m_Hovering =
        (Node::s_HoveredNodeID == GetUUID()) || (Node::s_HoveredNodeID == 0);
...

    {
        ...
        if (it != m_World.m_Chunks.end()) {
            ...
            m_HoveredElement =
                it->second->m_Elements[index.x + index.y * CHUNKSIZE];
        ...
        if (Input::IsMouseButtonPressed(0) && m_Hovering) {
            ...
            if (m_BuildingRigidBody) {
                if (pixelPos.x < m_RigidMin.x)
                    m_RigidMin.x = pixelPos.x;
                if (pixelPos.x > m_RigidMax.x)
                    m_RigidMax.x = pixelPos.x;
                if (pixelPos.y < m_RigidMin.y)
                    m_RigidMin.y = pixelPos.y;
                if (pixelPos.y > m_RigidMax.y)
                    m_RigidMax.y = pixelPos.y;
            } else {
                m_CurrentTickClosure.AddInputAction(
                    InputAction::Input_Place, (uint8_t)m_BrushSize,
                    (uint16_t)m_BrushType, (uint32_t)m_SelectedElementIndex,
                    pixelPos, false);
            }
        }
    }

    {
        ...
        // horizontals
        glm::vec2 worldMin = glm::vec2((float)m_RigidMin.x / CHUNKSIZEF,
                                       (float)m_RigidMin.y / CHUNKSIZEF);
        glm::vec2 worldMax = glm::vec2((float)m_RigidMax.x / CHUNKSIZEF,
                                       (float)m_RigidMax.y / CHUNKSIZEF);
        Renderer2D::DrawLine({worldMin.x, worldMin.y},
                             {worldMax.x, worldMin.y});
        Renderer2D::DrawLine({worldMin.x, worldMax.y},
                             {worldMax.x, worldMax.y});
        // vertical lines
        Renderer2D::DrawLine({worldMin.x, worldMin.y},
                             {worldMin.x, worldMax.y});
        Renderer2D::DrawLine({worldMax.x, worldMin.y},
                             {worldMax.x, worldMax.y});
    }
}
    ...
    for (int i = 0; i < tc.m_ClientCount; i++) {
        ...
        for (int i = 0; i < inputActionCount; i++) {
            ...
            case Pyxis::InputAction::Input_Place: {
                ...
                m_World.PaintBrushElement(pixelPos, elementID, brush,
                                          brushSize);
                break;
            }
            case InputAction::TransformRegionToRigidBody: {
                ...
                for (int x = -brushSize; x <= brushSize; x++) {
                    for (int y = -brushSize; y <= brushSize; y++) {
                        ...
                        case BrushType::circle:
                            ...
                            if (std::sqrt((float)(x * x) + (float)(y * y)) >=
                                brushSize)
                                continue;
                            break;
                ...
                if (pixels.size() > 0) {
                    ...
                    if (!makeCreature) {
                        def.type = PhysicsBody2DType::Dynamic;
                        m_World.CreatePixelBody(def, pixels, false,
                                                "F-Created RigidBody");
                    } else {
                        def.type = PhysicsBody2DType::Kinematic;
                        m_World.CreatePixelBody<Player>(def, pixels, true,
                                                        "F-Created Player");
                    }
                }
...
void GameNode::OnKeyPressedEvent(KeyPressedEvent &event) {
    if (event.GetKeyCode() == PX_KEY_Q) {
        ...
        if (m_World.ForceGetElement(pixelPos).m_ID != 0) {
            ...
            m_World.CreateParticle(pixelPos, {randX, randY},
                                   m_World.GetElement(pixelPos));
    ...
    if (event.GetKeyCode() == PX_KEY_C) {
        ...
        if (Input::IsKeyPressed(PX_KEY_F3)) {
            ...
            int type = ((int)m_BrushType) + 1;
            if (type >= (int)BrushType::end)
                type = (int)BrushType::end - 1;
            m_BrushType = BrushType(type);
        }
    }
}
...
/*void GameNode::OnWindowResizeEvent(WindowResizeEvent& event)
{
        ...
        m_Hotbar->SetScale((glm::vec3(1) / glm::vec3(windowSize.x / 2.0f,
windowSize.y / 2.0f, 1))); m_Hotbar->m_Size = { windowSize.x, windowSize.y *
0.1f }; m_Hotbar->m_TextureScale = 32; m_Hotbar->UpdateCanvasTransforms();
...
void GameNode::PaintBrushHologram() {
    ...
    for (int x = -m_BrushSize; x <= m_BrushSize; x++) {
        for (int y = -m_BrushSize; y <= m_BrushSize; y++) {
            ...
            case BrushType::circle:
                // limit to circle
                if (std::sqrt((float)(x * x) + (float)(y * y)) >= m_BrushSize)
                    continue;
                break;
            case BrushType::square:
            ...
            uint32_t color =
                ElementData::s_ElementData[m_SelectedElementIndex].color;
            ...
            Renderer2D::DrawQuad(
                (glm::vec3(newPos.x + 0.5f, newPos.y + 0.5f, 0.05)), {1, 1},
                vecColor);
        }
    }
}
